...
function luong($id)
{
  $sql = "select * from luongnhanvien where id_luong = '$id'";
    $conn = open_database();
    $query = mysqli_query($conn,$sql);
    ...
    if(mysqli_num_rows($query) >=1)
    {
      ...
      return array('luongcoban'=>$data['luongcoban'],'chisoluong'=>$data['chisoluong']);
    }
    mysqli_close($conn);
}
