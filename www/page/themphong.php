<?php 
    ...
    if(isset($_POST['adduser']))
    {
        ...
        if(empty($gia))
        {
            $giaerr = "Nhập Giá Phòng";
            array_push($error,$giaerr);
        }

        if($loai == -1)
        {
            $chucvuerro = "Chọn Loại Phòng";
            array_push($error,$chucvuerro);
        }

        if(empty($error)) 
        {
            $sql = "INSERT INTO phong(tenphong,giaphong,active,check_room)
            VALUES ('$name','$gia',0,'$loai')";
            $mysqli = open_database();
            $query = mysqli_query($mysqli,$sql);
            if($query){
                header("Location: ./quanlyphonghat.php?them=1");
            }
            mysqli_close($mysqli);
        }
    }
            ...
            <div class="management-user">
                ...
                <div class="add-user-header">
                        <div class="add-user-content">
                            <h2 class="add-user-title">Thêm Phòng</h2>
                        <form method="POST" action="">
                                <div class="form-input">
                                    <label for="username">Tên Phòng</label>
                                    <input class="bd-rd-rem" id="name" type="text" value="<?=$name?>"  placeholder="Nhập Tên Phòng" name="name">
                                </div>
                                ...
                                <div class="form-input">
                                    ...
                                    <input class="bd-rd-rem" id="gia" name = "gia" type="number"  value="<?=$gia?>" placeholder="Giá Phòng">
                                </div>
