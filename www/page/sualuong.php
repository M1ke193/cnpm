<?php 
    ...
    $idnv = isset($_GET['id'])? $_GET['id'] : null;
    if($idmonan == null || $idmonan == '') {
        header("Location: ./quanlyluong.php");
    }
    if($idnv == null || $idnv == '') {
        header("Location: ./quanlyluong.php");
    }
    ...
    if(isset($_POST['adduser']))
    {
        ...
        if(empty($gia))
        {
            $giaerr = "Nhập Lương Cơ bản";
            array_push($error,$giaerr);
        }
        if(empty($he))
        {
            $heerr = "Nhập Lương Cơ bản";
            array_push($error,$heerr);
        }

        if(empty($error)) 
        {
            $sql = "UPDATE luongnhanvien
                    SET luongcoban = '$gia', chisoluong = '$he'
                    WHERE id_luong = '$idmonan'";
            $mysqli = open_database();
            $query = mysqli_query($mysqli,$sql);
            if($query){
                header("Location: ./quanlyluong.php?chinhsua=1");
            }
            mysqli_close($mysqli);
        }
    }
            ...
            <div class="management-user">
                ...
                <div class="add-user-header">
                        <div class="add-user-content">
                            <h2 class="add-user-title">Chỉnh sửa Lương</h2>
                        <form method="POST" action="">
                                <div class="form-input">
                                    <label for="username">Họ tên</label>
                                    <input class="bd-rd-rem" id="name" type="text" value="<?=userid($idnv)['fullname']?>" disabled name="name">
                                </div>
                                <div class="form-input">
                                    <label for="cmnd">Lương cơ bản</label>
                                    <input class="bd-rd-rem" id="gia" name = "gia" type="number"  value="<?=luong($idmonan)['luongcoban']?>" placeholder="Giá Phòng">
                                </div>
                                ...
                                <div class="form-input">
                                    <label for="cmnd">Hệ số lương</label>
                                    <input class="bd-rd-rem" id="he" name = "he" type="number"  value="<?=luong($idmonan)['chisoluong']?>" placeholder="Giá Phòng">
                                </div>
                                <div class="alert alert-danger error-he" style="display:none">
                                    <?php
                                        if(!empty($heerr))
                                        {
                                            echo "<script>document.querySelector('.error-he').style.display = 'block'</script>";
                                            echo $heerr;
                                        }
                                    ?>
                                </div>
